...
(defun nu/mc--furthest-line-point (bottom-p)
  (require 'multiple-cursors-core)
  (let ((best (point))
        (best-line (line-number-at-pos)))
    (dolist (ov (overlays-in (point-min) (point-max)))
      (when (eq (overlay-get ov 'type) 'fake-cursor)
        (let* ((p (overlay-get ov 'point))
               (l (line-number-at-pos p)))
          (when (funcall (if bottom-p #'> #'<) l best-line)
            (setq best p best-line l)))))
    best))
...
(defun nu/mc-mark-next ()
  ...
  (require 'multiple-cursors-core)
  (let* ((col    (current-column))
         (anchor (nu/mc--furthest-line-point t))
         (target (save-excursion
                   (goto-char anchor)
                   (let ((orig (line-number-at-pos)))
                     (forward-line 1)
                     (when (> (line-number-at-pos) orig)
                       (move-to-column col)
                       (point))))))
    (if target
        (progn (save-excursion (goto-char target)
                               (mc/create-fake-cursor-at-point))
               (mc/maybe-multiple-cursors-mode))
      (message "Already on last line"))))

(defun nu/mc-mark-previous ()
  (interactive)
  (require 'multiple-cursors-core)
  (let* ((col    (current-column))
         (anchor (nu/mc--furthest-line-point nil))
         (target (save-excursion
                   (goto-char anchor)
                   (let ((orig (line-number-at-pos)))
                     (forward-line -1)
